...
namespace WIRECABLE
{
    ...
    public class Connector : MonoBehaviour
    {
        ...
        {
            if (this.ConnectedTo == null || onlyThis != null && onlyThis != this.ConnectedTo)
                return;

            Destroy(this.m_fixedJoint);

            // important to dont make recusrion
            Connector toDisconect = this.ConnectedTo;
            this.ConnectedTo = null;
            if (this.m_makeConnectionKinematic)
                toDisconect.Rigidbody.isKinematic = this.m_wasConnectionKinematic;
